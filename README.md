...
A Fabric 1.21.8 combat mod built around a small number of deeply designed Jujutsu-inspired character kits. The current playable vessels are Nobara (nails, hammer, Hairpin, Resonance, traps, Black Flash), Todo (Boogie Woogie swap, heavy melee, shared Black Flash bridge), and Megumi (Ten Shadows: Divine Dogs plus eight shikigami — Nue, Toad, Rabbit Escape, Max Elephant, Great Serpent, Round Deer, Piercing Ox, Tiger Funeral — with coexistence, a global sic, and a quick selector), plus a shared cinematic VFX pipeline and character menu.
...
- Megumi actions: R summons or recalls the currently selected shikigami (Divine Dogs by default); Shift+R issues one global sic — every living body across dogs and shikigami attacks the aimed eligible target, an empty aim clears orders; Shift+V cycles the selection and holding G opens the nine-slot quick selector. Summon and recall cooldowns are per shikigami type; the remaining time appears on Megumi's combat HUD.
