...
final class ProjectJjkNailEmbedding {
	...
	static Vec3 bodyEmbedPoint(Vec3 targetPosition, double targetWidth, double targetHeight, Vec3 hitPoint, Vec3 direction, int seed) {
		...
		Vec3 forward = safeDirection(horizontalForward);
		...
		double bodyRadius = Math.max(0.18, targetWidth * 0.42);
		...
		return targetPosition
				.add(0.0, y, 0.0)
				.subtract(forward.scale(bodyRadius))
