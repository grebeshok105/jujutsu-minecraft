...
public final class NobaraVfxRecipes {
	...
	private static VfxInstance hammer(VfxCue cue) {
		return VfxInstance.of(10, (context, initialAgeTicks) -> {
			...
			context.camera().triggerSwing(intensity(cue), proximity);
			context.hud().triggerSwing(proximity);
		});
	}
	...
	private static VfxInstance impact(VfxCue cue) {
		return VfxInstance.of(20, (context, initialAgeTicks) -> {
			...
			context.camera().triggerImpact(intensity(cue), proximity);
			context.hud().triggerImpact(proximity);
		});
	}

	private static VfxInstance impactSound(VfxCue cue) {
		return VfxInstance.of(2, (context, initialAgeTicks) -> playImpactSound(context, context.resolveOrigin(cue), 1.0f, random(cue, 0x41E7L)));
	}
	...
	private static VfxInstance resonanceChannel(VfxCue cue) {
		return VfxInstance.of(12, (context, initialAgeTicks) -> {
			...
			if (proximity > 0.01f) {
				context.camera().triggerImpact(intensity(cue), proximity);
				context.hud().triggerImpact(proximity * 0.55f);
			}
		});
	}
	...
	private static VfxInstance resonanceStrike(VfxCue cue) {
		return VfxInstance.of(24, (context, initialAgeTicks) -> {
			...
			if (proximity > 0.01f) {
				context.camera().triggerImpact(marks, proximity);
				context.hud().triggerImpact(proximity);
			}
		});
	}
	...
	private static VfxInstance detonate(VfxCue cue) {
		return VfxInstance.of(14, (context, initialAgeTicks) -> {
			...
			if (proximity > 0.01f) {
				context.camera().triggerSwing(marks, proximity * 0.7f);
				context.hud().triggerSwing(proximity * 0.65f);
			}
		});
	}
	...
	private static VfxInstance enlarge(VfxCue cue) {
		return VfxInstance.of(28, (context, initialAgeTicks) -> {
			...
			if (proximity > 0.01f) {
				...
				context.camera().triggerImpact(marks + 2, Math.min(1.0f, proximity * 1.15f));
				context.hud().triggerImpact(Math.min(1.0f, proximity * 1.1f));
			}
		});
	}
	...
	private static VfxInstance explosion(VfxCue cue) {
		return VfxInstance.of(18, (context, initialAgeTicks) -> {
			...
			if (proximity > 0.01f) {
				...
				context.camera().triggerImpact(marks, Math.min(1.0f, proximity * 0.82f));
				context.hud().triggerImpact(Math.min(1.0f, proximity * 0.74f));
			}
		});
	}

	private static VfxInstance firstPersonSnap(VfxCue cue) {
		return VfxInstance.of(15, (context, initialAgeTicks) -> context.firstPerson().triggerSnap());
	}
