...
public final class ServerTimeDilationTest {
	...
	public static void main(String[] args) throws Exception {
		...
		Class<?> accessType;
		try {
			controllerType = Class.forName("jujutsu.mod.character.nobara.projectjjk.ServerTimeDilation");
			accessType = Class.forName("jujutsu.mod.character.nobara.projectjjk.ServerTimeDilation$TickRateAccess");
		} catch (ClassNotFoundException missing) {
			throw new AssertionError("Missing server-authoritative Resonance time dilation controller", missing);
		}
		...
		for (int index = 0; index < 3; index++) tick.invoke(controller, rate.proxy());
		...
		trigger.invoke(controller, rate.proxy(), 6.0f, 4);
		for (int index = 0; index < 4; index++) tick.invoke(controller, rate.proxy());
