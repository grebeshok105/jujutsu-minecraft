...
public final class NailAnchorTest {
	...
	public static void main(String[] args) {
		...
		NailAnchor block = NailAnchor.block(new BlockPos(4, 70, -3), ResourceLocation.parse("minecraft:overworld"), Direction.UP, "minecraft:stone", new Vec3(0.25, 0.5, 0.75), new Vec3(0.0, 1.0, 0.0));
		assert block.kind() == NailAnchor.Kind.BLOCK;
		assert block.blockPos().equals(new BlockPos(4, 70, -3));
		assert block.dimension().equals(ResourceLocation.parse("minecraft:overworld"));
		assert block.face() == Direction.UP;
		...
		assert registry.resolve(unknown, Vec3.ZERO).resolution() == NailAnchorResolution.TEMPORARILY_UNAVAILABLE;
