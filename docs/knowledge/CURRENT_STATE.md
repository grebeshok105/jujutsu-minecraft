...
- Первая world-space SDF сфера (будущий Domain Expansion): fullscreen raymarch post-effect, порт central-sphere техники Orbital Railgun (MIT) на MC 1.21.8 RenderPipeline/RenderPass/std140 UBO, без Satin. Rule-of-four прошёл полностью: 4 скаута → план (R1-R21) → 3 ревью плана → 4 воркера → барьер → 4 ревьюера + QA → адъюдикация → judge VERIFIED → in-game → PR.
- Архитектура: `DomainSphereTiming` (pure math, easeOutQuart 20t / hold 220t / fade-collapse 20t) + `VfxDomainSphereChannel` (cap 4, evict oldest, no GL/ивентов в ctor) + `DomainSphereRenderer` (ленивые GL-объекты, color+depth копии, 160B UBO на сферу, ≤4 draw). Хук `WorldRenderEvents.LAST` → `VfxDirector.renderLast`. Триггеры: `/jujutsu_debug domain_sphere [5..64]` + mcpdev `jujutsu_domain_sphere`.
- **Грабли (проверены живьём)**: (1) ByteBuffer, переиспользуемый между кадрами, обязан re-arm лимита — `data.limit(count*stride)` без `clear()` → BufferOverflowException на кадре с большим count (пойман тремя ревьюерами независимо); (2) клиентский командный рут, совпадающий с серверным (`jujutsu`), затеняет ВСЕ серверные сабкоманды — Brigadier кидает dispatcherUnknownArgument, который fabric-command-api-v2 НЕ форвардит; (3) `VesselBoundaryTest` считает каждый дочерний пакет `client.vfx.` сосудом — shared non-vessel пакеты требуют записи в SHARED_SEGMENTS_BY_PARENT + javadoc-обоснования; (4) синтетический ввод (keybd_event/PostMessage/SendInput) не доходит до окна игры — F3+T не проверяемо автоматом.
- **Фиксы по фидбеку пользователя** (2bb3d6e): fade-рассинхрон (shockwave не умножался на fade → кольцо догорало отдельно) → shockwave×fade + радиус схлопывается в 0; интерьер cyan-залив (ring до ~100 + 10/dist²) → ring≤2.0, total≤1.5. **Затем 190e467**: клампы убиты и неоновое кольцо — кольцо и есть тот же ring-член; финальное состояние = яркость первого PR без клампов, fade-sync сохранён (shockwave×fade + collapse радиуса). Урок: «wash интерьера» и «нужное кольцо» — один терм, гасить по времени/маске, не по потолку.
- Проверено: qualityGate зелёный (310 GameTest/0, JUnit 595), in-game: depth copy OK, сфера рендерится, два триггера без session-disable, resize чистый, collapse покадрово. Не проверено: F3+T mid-effect (ручная проверка).
