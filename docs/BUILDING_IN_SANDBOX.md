...
The gate does not build a jar. For the remapped artifact at `build/libs/jujutsumod-<version>.jar` (the version comes from `mod_version` in `gradle.properties`):
...
Migration of the existing JavaExec programs is deliberate and gradual. The four classes that own mutable static state — CharacterAbilityCooldowns, CombatStagger, NailAnchorRegistry, ProjectJjkNailMarks — must move last: today each JavaExec program is its own JVM, so state cannot leak between tests, and a shared JUnit JVM removes that guarantee.
