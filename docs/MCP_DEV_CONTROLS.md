...
1. **Tool surface grew past the v0 seven.** Issue #43's "~12-18 tools" describes the eventual L3 orientation; v0 shipped the minimal closed loop (`jujutsu_vessel_list`, `jujutsu_vessel_select`, `jujutsu_ability_invoke`, `jujutsu_cooldowns_get`, `jujutsu_cooldowns_clear`, `jujutsu_fixture_reset`, `jujutsu_state_get`). Since then the surface grew: `jujutsu_ticks_wait`, `jujutsu_player_set_rotation`, `jujutsu_fixture_list`, `jujutsu_mod_status`, and the 15 `jujutsu_incident_*` tools below (issue #110). Everything else already exists upstream (105 tools) or waits for a proven need.
