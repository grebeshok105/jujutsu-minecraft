...
**What a green gate proves, and what it does not.** It proves the shape of the code, the contracts between vessels, and the pure logic reachable without a world — and it boots a headless server for two neutral GameTest canaries, so a `ServerLevel` does exist in the suite. It does **not** prove gameplay: no automated check casts an ability or moves a body. An optional, non-required client GameTest lane exists (`runClientGameTest`, and the `client-gametest` `workflow_dispatch` CI experiment) — it renders frames and takes screenshots, but it proves no gameplay and is not part of the gate. Ability/world behaviour coverage remains E1 in [docs/KNOWN_ISSUES.md](docs/KNOWN_ISSUES.md) and the issue #21 scenario list; in-world behaviour is still proven only by the client-smoke checklist. Do not claim in-game behavior from a green gate.
