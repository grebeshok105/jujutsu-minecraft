...
Megumi is the third vessel. `PRIMARY` (`R`) summons or recalls one pack containing two separately mortal `MegumiDivineDogEntity` wolves; `PRIMARY_SNEAK` (`Shift+R`) assigns one server-selected Sic target to every living sibling. This note owns the Divine Dogs slice; the `SECONDARY`/`SECONDARY_SNEAK` shadow abilities added later are owned by [Megumi shadow kit](Megumi-shadow-kit.md). The dogs slice itself added no items, persistence, payload, input, mixin, dependency or universal summon abstraction.
...
The vanilla wolf model, variants and textures remain in use. A summon emits `DOGS_SUMMON_BODY` for Megumi's `summon_divine_dogs` GeckoLib clip and one `DOGS_SUMMON` exact-origin cue per dog for its shadow pool; a failed client anchor lookup can therefore never turn a body cue into a second pool. Recall emits one cue per still-living dog before the phase starts. `MegumiVfxRecipes` owns the five Divine Dogs ids — `DOGS_SUMMON_BODY`, `DOGS_SUMMON`, `DOGS_RECALL`, `DOGS_SIC` and `DOGS_POUNCE` — alongside the shadow-kit ids listed in [Megumi shadow kit](Megumi-shadow-kit.md). The VFX Core world channel draws summon/recall as a filled, alpha-blended black pool from independent quad sectors, without a teal edge ring or shared triangle-fan geometry; the dedicated `megumi_shadow_spot` sprite backs a 1-in-10 neutral near-black accent mote, and all motes use world light.
...
Each dog owns unsynchronized, non-persistent Sic/pounce UUIDs and deadlines. Immediately before launch the server requires an `ACTIVE` current-pack dog, live loaded same-level owner, exact current Sic target, eligibility, line of sight, inclusive 3.0-8.0 block range and that dog's readiness. Launch starts at 0.92 horizontal and 0.42-0.58 vertical speed, then server-side steering keeps the horizontal motion on the live target without overwriting ballistic vertical motion. A horizontal or vertical collision, a landing after the first actual movement tick, timeout, invalidation or teardown finishes the flight and restores ordinary ACTIVE AI where appropriate. An ordinary airborne finish keeps only the damped horizontal component of the actual post-move displacement; an ordinary grounded finish, invalid semantic contact and cleanup finish at zero motion. Launch starts only that dog's 80-tick cooldown.

With AI disabled during flight, the server owns pounce motion explicitly: each tick it steers the horizontal velocity, applies profile gravity, moves through `MoverType.SELF`, updates facing, derives actual post-move displacement from the position before and after `move()` (because `getDeltaMovement()` is not collision-resolved by `move()`), and then evaluates real collision flags. Impact uses the target's inflated swept AABB plus endpoint overlap, so a fast dog cannot tunnel through a target between ticks. A valid swept impact intentionally has priority over landing or collision from the same post-move tick; otherwise those conditions finish the flight. The server revalidates owner, target, pack command and friendly-fire policy, then applies exactly one `playerAttack(owner)` hit for base 3.0 plus 2.0 pounce damage. Only accepted damage applies a 2.4-strength horizontal knockback based first on the post-move displacement, then on the pre-cleanup positional fallback, `CombatStagger.GLOBAL` for 6 ticks, the spatial impact sound and target-anchored `DOGS_POUNCE` cue. Only a valid semantic contact keeps damped horizontal exit motion; invalid contact uses ordinary zero cleanup. Completed ordinary flight and valid contact resume navigation only through the runtime transition when the captured Sic command and target are still current and eligible; recovery calls vanilla navigation with the explicit `NAVIGATION_SPEED_MODIFIER` of 1.0, separate from the 0.34 movement attribute. A miss, invalidation, phase change or teardown emits no hit feedback or residual motion; an airborne timeout is an ordinary finish and keeps only its damped post-move displacement. Inherited ordinary wolf melee attribution remains unchanged and is outside this polish pass.
