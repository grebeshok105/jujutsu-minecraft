...
The caster must receive the complete scene even when the remote target is a mob with no client screen. `DOLL_STRIKE` therefore keeps a 24-tick effigy/hammer phase and gives local Nobara a `0.45x` render-time pulse, a visible nausea wash, extended blur, zoom, and shake. `RESONANCE_RELEASE` keeps a 38-tick dark-center/cyan-fracture phase; a player who is actually the target still receives its additional target-local reaction. Observers retain the shared world impact and proximity-scaled camera response. `VfxTimeChannel` scales render partial ticks only, never server ticks or gameplay authority (`NobaraVfxRecipes.java`; `VfxCameraChannel.java`; `VfxHudChannel.java`; `VfxDeltaTrackerMixin.java`).
