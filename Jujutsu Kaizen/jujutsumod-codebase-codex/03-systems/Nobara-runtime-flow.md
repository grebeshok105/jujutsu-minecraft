...
ProjectJjkRitualRuntime obtains loaded nails from EmbeddedNailRegistry, orders a HairpinChain (`HairpinChain`/`HairpinChainOrder`/`HairpinChainScheduler` — chain shape, ordering, cadence), resolves concrete nail entities, applies dedicated Hairpin damage, and discards resolved nails. `NailAnchor`/`NailAnchorResolution`/`NailRuntimeAnchorRegistry` own the embedded-anchor identity the mega path matches on; `NobaraActionGuard` is the router-level re-entrancy guard and `NobaraActionTimeline` centralizes windups. R seed search prefers anchors ahead of the cast start (forward filter `along >= 0.0`).

B is the Mega Nail (`ProjectJjkMegaNailRuntime.start`): it resolves the aimed target through `TargetResolver` within `HAIRPIN_ENLARGE_RANGE`, selects that target's embedded owned nails by `anchor().stableId()` equality, atomically discards them (per-nail consume flash) and consumes the target's marks, then fires an entity-driven charge (`MEGA_NAIL_CHARGE_TICKS` 24) and flight (timeout 60) whose homing resolves by UUID and whose damage scales from the consumed depth weight (4.0/nail, cap 42). The aim here is the mechanic, not a regression: the old massless "detonate everything loaded" B was replaced wholesale. An empty B (no aimed target with your nails) returns false and shows the router's fallback message; an empty R stays a consume-free refusal.
