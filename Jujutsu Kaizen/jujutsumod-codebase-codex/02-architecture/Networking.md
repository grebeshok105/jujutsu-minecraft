...
Nine typed custom payloads are registered:
...
| C2S | CharacterAbilityPayload | shared primary ability slot (Todo Boogie Woogie) |
...
C2S receivers execute on the server thread. SelectCharacterPayload limits ids to 32 UTF characters. Unknown character ids map to None; unknown action ids fail closed. CharacterAbilityPayload carries only a slot id; the server resolves character, target, and success. CurseLinkOptionsPayload still needs list/string bounds.
