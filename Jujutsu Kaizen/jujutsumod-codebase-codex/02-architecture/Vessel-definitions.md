...
**`selectCurseLink(player, linkId)`** — answers a curse-link choice the client just made, defaulting to a refusal. A curse link is a shared concept, so the packet carries a neutral intent — *the player picked link X* — and the vessel decides what that means; Nobara forwards it to `SelfResonanceRuntime.select`, which still validates ownership itself. This hook exists because the receiver used to call that runtime by name from `JujutsuNetworking`, the seam breach tracked as E13. Routing it here also closed a defect nobody was tracking: a receiver that names one vessel's runtime honours the packet from *any* sender, while the seam refuses a sender who is not that vessel. See [Networking](Networking.md).
