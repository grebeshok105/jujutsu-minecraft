...
- Current cards: Nobara, Todo, Megumi, and None, in roster order — drawn from `JujutsuCharacterClients.inRosterOrder()`, so the panel holds no vessel's data and a new vessel appears with no edit here (VERIFIED — `CharacterRosterPanel.CARDS` is initialized from the registry). Each card is the vessel's own `rosterEntry()`: nameKey/roleKey/subtitleKey, portrait, and an input strip listing what its router actually answers.
- Nobara, Todo, and Megumi cards carry a skin portrait read from each definition's `playerSkin()` (`textures/entity/character/nobara.png`, `.../todo.png`, `.../megumi.png`); None is the vanilla-play card.
