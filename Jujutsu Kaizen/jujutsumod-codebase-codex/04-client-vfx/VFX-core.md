...
docs, tests, and string literals cannot satisfy it. The radius inventory reuses that production graph
and resolves named delivery fields from the actual network path, including helper calls and multi-site
owners. `DOGS_SUMMON` keeps one player-anchored cue for the body animation and adds one exact-origin
dog-anchored cue per dog for the 16-tick shadow-open pool; `DOGS_RECALL` is likewise one still-living
dog cue per 12-tick shadow-close pool, never a synthetic owner pool. `DOGS_POUNCE` is a target-anchored
ring/burst emitted only after accepted server damage. The shared world channel owns both bounded ground
styles and Megumi's client definition registers the shadow-mote provider. ProjectSanityTest requires
age-aware real-time channel calls and rejects removed legacy managers/mixins. Six client mixins are
...
Effects use cue age to reject or seek late playback rather than replaying stale beats from the start. Persistent visuals are not VFX Core's job: nails are drawn by `ProjectJjkNailRenderer` (see [Nail rendering](Nail-rendering.md)), and the target-info overlay was a VfxDirector HUD contribution until it was archived on 2026-09-09 (`archive/combat-hud-v1`), while transient compression, snap, burst, residue, camera, and sound beats belong to recipes and channels.

`VfxDirector` registers exactly one HUD element — `HudElementRegistry.attachElementAfter(VanillaHudElements.MISC_OVERLAYS, jujutsumod:vfx_overlay, VfxDirector::renderHud)` (VERIFIED). Adding a second `HudRenderCallback` for one ability, or a per-effect HUD singleton, is forbidden by this contract; so is a new mixin for a single flash, and so is sending any gameplay packet from HUD code. Vessels plug into that one element through the fan-out API `VfxDirector.registerHudContribution(id, contribution)` (VERIFIED — VfxDirector.java:68): four live contributions remain: `megumi_divine_dogs_cooldown`, `megumi_shadow_dive_veil`, `todo_pair_status`, `todo_stone_status` — `ability_hud` and `nobara_target_hud` were archived with the combat HUD on 2026-09-09 (`archive/combat-hud-v1`).

The shared ability strip — `AbilityHud`, a bottom-center SDF/MSDF strip with 20px cells and a session-only drag position — was ARCHIVED on 2026-09-09 (`archive/combat-hud-v1`): both registration points (the `ability_hud` VfxDirector contribution in JujutsuModClient and the `END_CLIENT_TICK` drag hook in its static block) are removed, so no ability strip renders in the active build. Per-vessel content came from the `hudSlots()` + `maxCooldownTicks()` seam on `CharacterClientDefinition` (CharacterClientDefinition.java:29-40), which is kept for a clean restore — every vessel still implements `hudSlots()`, Todo and Megumi override `maxCooldownTicks()` (Nobara's default-0 question is resolved as moot, see Uncertainties). The remaining per-vessel status HUDs (`TodoStatusHud`, `MegumiCooldownHud`, `MegumiShadowDiveHud`) are separate contributions on the same path, not separate elements; `NobaraTargetHud` was archived with the strip.
