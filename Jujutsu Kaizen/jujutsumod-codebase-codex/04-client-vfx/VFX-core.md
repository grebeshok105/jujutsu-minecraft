...
| Black Flash | `black_flash` | directional slash blades (5) + 3 lightning variants (forked/spiral/cascade, seed%3) + expanding shockwave ring + inner blood ring (world, 28 ticks, fixed in world space), BF custom particles (impact/lightning/spark + directional bursts), 5-layer sound (impact x2 + snap + deep explosion + vortex rumble), aggressive 4-impulse camera shake (~560ms + aftershock), HUD flash 250ms + nausea 0.8, blur 300ms, first-person snap (caster only), GeckoLib animation. Blood-black palette (core 80/30/30, additive-safe). | VERIFIED |
