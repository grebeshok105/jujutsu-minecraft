...
`src/client/resources/assets/jujutsumod/shaders/core/` holds 28 files (VERIFIED — directory listing). Three families are wired through the adapters: `msdf.*` (MSDF text), `sdf_shape.*` (SDF panels), and `sdf_glass.*` (liquid-glass refraction via `SceneSampler`, per-shape opt-in through a negative highlight — VERIFIED — SdfPipelines.java:70-75). The rest — `rect.*`, `blur.*`, `glass_composite.*`, `kawase_down.*`, `kawase_up.*`, `outline.*`, `glow_outline.*`, `mask_diff.*`, `arc_*`, `texture.*` — are port artifacts kept for research. Honest state: `sdf_glass` is infrastructure with a passing canary (`SdfGlassCanaryTest`) but no production HUD consumer — the combat HUD that rode the SDF/MSDF stack (`AbilityHud`) was archived on 2026-09-09 (`archive/combat-hud-v1`), and nothing else in the HUD path uses glass, so the glass pipeline currently has no HUD path to ride. Do not present it as the active HUD renderer.
