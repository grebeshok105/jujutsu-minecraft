...
Target ESP rode the renderer until 2026-08-21; the billboard and leader nail were removed and the overlay became a screen-space HUD contribution (`NobaraTargetHud`). The whole overlay — HUD, ESP state scan, layout/animation, rank classification — was ARCHIVED on 2026-09-09 (`archive/combat-hud-v1`) and unregistered, so no target HUD renders. `ProjectJjkNailRenderer` keeps only the accent-colour pulse on locally-owned embedded nails (`ownedByLocal`); its tracked shared-code references are pinned by `SourceBoundaryTripwireTest` (E14) — three entries: `ProjectJjkNailEmbedding`, `ProjectJjkNailEntity`, `<package nobara>`.
